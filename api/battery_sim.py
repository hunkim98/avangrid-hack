...
class Battery:
    def __init__(self, wattage_mw, duration_h, cycle_life, cycle_age):
        ...
        self.capacity = max_capacity * (
            1 - (cycle_age / cycle_life) * 0.2
        )  # Cycle life defined to 80% original capacity
    ...
    def charge(self, rate):
        ...
        if (
            rate > self.max_rate
        ):  # If available MWh exceeds how much can be charged in 1 hr, we lose that power
        ...
        if self.stored + rate >= self.capacity:
            ...
            )  # any excess power that can't fit in battery is overflow
...
class YearSim:
    def __init__(
        ...
        file_path = Path(__file__).parent / f"{site}.csv"
...
def revenue_calc(
    ...
    first_year = YearSim(
        site,
        battery_wattage=battery_power_MW,
        battery_duration=battery_duration_H,
